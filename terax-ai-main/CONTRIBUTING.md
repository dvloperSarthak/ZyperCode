# Contributing to ZyperCode

ZyperCode is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- ZyperCode has one active maintainer ([@crynta](https://github.com/crynta)).
...
ZyperCode positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
The most common way a PR breaks ZyperCode is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
## What ZyperCode is not

To set expectations:

- ZyperCode is terminal-first, not an IDE clone. Focused capabilities such as LSP, AI autocomplete, formatting, source control, and previews belong when they stay fast, lazy, and resource-bounded.
