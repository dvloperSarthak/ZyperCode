...
ZyperCode 是一个轻量、开源、终端优先的 AI 原生开发环境（ADE），基于 Tauri 2 + Rust 和 React 19 构建。它内置原生 PTY 后端与 WebGL 渲染器、使用你自己的密钥或完全本地模型运行的智能体 AI 侧边栏，以及代码编辑器、文件浏览器、带 Git 图的源代码管理和网页预览面板。磁盘占用约 7-8 MB。无遥测。无需账户。
...
最新安装程序位于 [Releases](https://github.com/crynta/terax-ai/releases/latest) 页面。ZyperCode 会从该页面自动更新。
...
1. 打开**设置 -> AI**。
2. 选择提供商并粘贴 API 密钥。对于本地推理，将 ZyperCode 指向你的 LM Studio / MLX / Ollama 端点。
...
ZyperCode 使用 Apache-2.0 许可证。有关依赖项的更多信息，请参阅 [Apache License 2.0](../../LICENSE)。
