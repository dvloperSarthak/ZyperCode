...
fn apply_common(
    ...
    cmd.env("ZYPERCODE_TERMINAL", "1");
    cmd.env("TERAX_TERMINAL", "1");
    if blocks {
        cmd.env("ZYPERCODE_BLOCKS", "1");
    ...
    if let Some(control) = control {
        cmd.env("ZYPERCODE_CONTROL_ADDR", &control.address);
        cmd.env("TERAX_CONTROL_ADDR", &control.address);
        cmd.env("ZYPERCODE_CONTROL_TOKEN", &control.token);
        cmd.env("TERAX_CONTROL_TOKEN", &control.token);
        cmd.env("ZYPERCODE_PANE_ID", control.pane_id.to_string());
        cmd.env("TERAX_PANE_ID", control.pane_id.to_string());
        if let Some(path) = &control.cli_path {
            cmd.env("ZYPERCODE_CLI", path);
