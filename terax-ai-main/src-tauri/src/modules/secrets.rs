...
pub async fn secrets_get(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let found = with_store(&app, &state, |m| m.get(&key).cloned())?;
        if found.is_some() {
            return Ok(found);
        }
        if service == "zypercode-ai" {
            let legacy_key = key("terax-ai", &account);
            let legacy_found = with_store(&app, &state, |m| m.get(&legacy_key).cloned())?;
            if let Some(ref val) = legacy_found {
                let _ = with_store(&app, &state, |m| m.insert(key, val.clone()));
            }
            return Ok(legacy_found);
        }
        Ok(None)
    }
    #[cfg(not(target_os = "linux"))]
    {
        ...
        match e.get_password() {
            ...
            Err(keyring::Error::NoEntry) => {
                if service == "zypercode-ai" {
                    if let Ok(legacy_entry) = entry("terax-ai", &account) {
                        if let Ok(legacy_val) = legacy_entry.get_password() {
                            let _ = e.set_password(&legacy_val);
...
pub async fn secrets_get_all(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        Ok(accounts
            .into_iter()
            .map(|a| {
                let current = keyring::Entry::new(&service, &a)
                    .ok()
                ...
                if service == "zypercode-ai" {
                    if let Ok(legacy_entry) = keyring::Entry::new("terax-ai", &a) {
                        if let Ok(legacy_val) = legacy_entry.get_password() {
                            if let Ok(new_entry) = keyring::Entry::new(&service, &a) {
                                let _ = new_entry.set_password(&legacy_val);
                            }
                            return Some(legacy_val);
                        }
                    }
                }
                None
