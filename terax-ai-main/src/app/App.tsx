...
function IdeShell() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (
        ...
        e.preventDefault();
        window.dispatchEvent(new CustomEvent("terax:open-folder-dialog"));
      }
    };
    ...
    (request: AgentLaunchRequest) => {
      ...
      const hooksReady = launcher.supportsHooks
        ? invoke("agent_enable_hooks", {
            ...
            console.warn(
              `[zypercode] could not enable ${request.agent} notifications:`,
              error,
            );
          })
        : Promise.resolve();
      ...
      for (const leafId of agentLeafIds) {
        void (async () => {
          ...
          if (!writeToSession(leafId, `${command.command}\r`)) {
            console.error(
              `[zypercode] agent terminal ${leafId} closed before launch`,
            );
          }
        })();
      }
  ...
  useEffect(() => {
    ...
    let disposed = false;
    (async () => {
      const handler = (e: { payload: string[] }) => openLaunchFiles(e.payload);
      const [off1, off2] = await Promise.all([
        listen<string[]>("zypercode:open-file", handler),
        listen<string[]>("terax:open-file", handler).catch(() => () => {}),
      ]);
      if (disposed) {
        off1();
        off2();
  ...
  useEffect(() => {
    const handleOpenFileEvent = (e: Event) => {
      const customEvent = e as CustomEvent<{ path: string; pin?: boolean }>;
      if (customEvent.detail?.path) {
        handleOpenFile(customEvent.detail.path, customEvent.detail.pin ?? true);
      }
    };
    window.addEventListener("zypercode:open-file", handleOpenFileEvent);
    return () =>
      window.removeEventListener("zypercode:open-file", handleOpenFileEvent);
  }, [handleOpenFile]);
