...
function getExtensionForLang(lang: string): string {
  ...
  return map[l] || (l.length > 0 && l.length < 8 ? l : "txt");
}

function OpenInEditorButton({ code, lang }: { code: string; lang: string }) {
  const [opened, setOpened] = useState(false);
  const onOpen = async () => {
    try {
      const root = useChatStore.getState().live.getWorkspaceRoot();
      const ext = getExtensionForLang(lang);
      const filename = `code_${Date.now().toString().slice(-4)}.${ext}`;
      const fullPath = root ? `${root.replace(/[\\/]$/, "")}/${filename}` : filename;
      const { invoke } = await import("@tauri-apps/api/core");
      await invoke("fs_write_file", { path: fullPath, content: code });
      window.dispatchEvent(
        new CustomEvent("zypercode:open-file", {
          detail: { path: fullPath, pin: true },
        }),
      );
      setOpened(true);
  ...
  return (
    <Button
      ...
      title="Open code as file in editor"
    >
      <HugeiconsIcon icon={opened ? CheckmarkCircle01Icon : FilePlusIcon} size={11} strokeWidth={1.75} />
      <span>{opened ? "Opened" : "Open in Editor"}</span>
    </Button>
  );
}
    ...
    <div className="not-prose my-2 overflow-hidden rounded-lg border border-border/50 bg-muted/30">
      <div className="flex items-center justify-between gap-2 border-b border-border/40 bg-muted/20 px-3 py-1">
        ...
        <div className="flex items-center gap-1">
          <OpenInEditorButton code={code} lang={label} />
