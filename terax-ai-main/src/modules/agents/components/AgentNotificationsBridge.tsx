...
export function AgentNotificationsBridge({
  ...
  useEffect(() => {
    let alive = true;
    const unsubs: (() => void)[] = [];
    const cb = (e: { payload: AgentSignal }) => handleSignal(e.payload, ctxRef.current);
    Promise.all([
      listen<AgentSignal>("zypercode:agent-signal", cb).then((u) => { if (alive) unsubs.push(u); else u(); }),
      listen<AgentSignal>("terax:agent-signal", cb).then((u) => { if (alive) unsubs.push(u); else u(); }).catch(() => {}),
    ]).catch(() => {});
    return () => {
      alive = false;
      unsubs.forEach((u) => u());
