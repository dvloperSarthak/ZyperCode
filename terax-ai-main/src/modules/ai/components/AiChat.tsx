  ...
  return (
    <Conversation>
      <ConversationContent className="gap-5 p-3">
        ...
        {error && (
          <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3.5 text-xs text-destructive flex flex-col gap-2 shadow-sm">
            <div className="flex items-center gap-2 font-semibold text-[13px] text-destructive">
              <HugeiconsIcon icon={AlertCircleIcon} size={16} strokeWidth={2} className="shrink-0" />
              <span>
                {error.message.includes("Provider") || error.message.includes("502") || error.message.includes("attempts")
                  ? "AI Provider Error (502 / Upstream Failure)"
                  : "Request failed."}
              </span>
            </div>
            <div className="leading-relaxed text-destructive/90 text-[11.5px]">
              {error.message}
            </div>
            <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-destructive/20 mt-0.5">
              <Button
                size="sm"
                variant="outline"
                className="h-7 text-[11px] border-destructive/40 bg-background/80 hover:bg-background text-foreground gap-1.5"
                onClick={() => {
                  clearError();
              ...
              <Button
                size="sm"
                className="h-7 text-[11px] bg-primary text-primary-foreground hover:opacity-90 gap-1.5 font-medium shadow-sm"
                onClick={() => {
                  clearError();
                  const targetModel = "meta-llama/llama-3.3-70b-instruct:free";
                  void useChatStore.getState().setSelectedModelId(targetModel);
                  void import("@/modules/settings/store").then(({ setOpenrouterModelId }) => {
                    void setOpenrouterModelId(targetModel);
                  });
                  const lastUser = [...messages].reverse().find((m) => m.role === "user");
                  if (lastUser) {
                    const text = lastUser.parts.find((p) => p.type === "text")?.text || "";
              ...
              >
                ...
                Switch to Llama 3.3 70B &amp; Retry
              </Button>
              <Button
                size="sm"
                variant="ghost"
                className="h-7 text-[11px] text-muted-foreground hover:text-foreground ml-auto"
                onClick={clearError}
              >
                Dismiss
              </Button>
