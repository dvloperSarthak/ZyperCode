...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "ZyperCode needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "ZyperCode run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "ZyperCode finished", "Your task is ready");
    }
  }, [status, error]);
