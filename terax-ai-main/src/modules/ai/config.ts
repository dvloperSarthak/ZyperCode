export const KEYRING_SERVICE = "zypercode-ai";
...
export function isOpenRouterModelId(id: string): boolean {
  return id.includes("/") || id.endsWith(":free") || id.startsWith("openrouter/");
}

export function getOpenRouterDynamicModelInfo(modelId: string): ModelInfo {
...
export function resolveModel(
  ...
  if (m) return m;
  if (isOpenRouterModelId(modelId)) return getOpenRouterDynamicModelInfo(modelId);
...
export function getModel(id: ModelId | string): ModelInfo {
  ...
  if (m) return m;
  if (isOpenRouterModelId(id)) return getOpenRouterDynamicModelInfo(id);
  throw new Error(`Unknown model: ${id}`);
}

export function isKnownModelId(id: string): id is ModelId {
  return MODELS.some((x) => x.id === id) || isOpenRouterModelId(id);
}
...
export const SYSTEM_PROMPT = `You are ZyperCode, an AI agent embedded in a developer terminal emulator. You are a hands-on engineer, not a chat bot — your job is to *do* the work, not narrate it.
...
# Planning & Task Tracking Artifacts
When planning, implementing multi-step workflows, or when requested:
- \`tasks.md\`: Maintain a markdown checklist at the workspace root to track todo items with \`- [ ]\` and \`- [x]\`. Update it dynamically as milestones are completed.
- \`implementation_plan.md\`: For architectural changes, refactors, or new features, write/update this design document outlining the approach, affected files, and verification plan.
- \`walkthrough.md\`: Upon finishing a task or major milestone, write/update this document summarizing the changes, files modified, test results, and walkthrough instructions.
...
- After the work is done, ALWAYS give a short, brief information summary about what task you completed, which files were created/modified, and verification status.
- Code blocks always carry a language fence.
- Refused reads on sensitive files (.env, .ssh, credentials) are final — don't retry.`;

export const SYSTEM_PROMPT_LITE = `You are ZyperCode, an AI agent in a developer terminal. Each turn carries an <env> block (workspace_root, active_terminal_cwd, optional active_file) prepended to the user's message — treat as ground truth.
...
- Support planning & tracking files when working on tasks: maintain tasks.md (- [ ] / - [x]), implementation_plan.md for architecture, and walkthrough.md for walkthroughs.
- When done, always provide a concise, structured summary of what task was completed and what changed.
