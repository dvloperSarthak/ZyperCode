  ...
  switch (provider) {
    ...
    case "openrouter": {
      ...
      built = createOpenAICompatible({
        ...
        apiKey: key,
        headers: {
          "HTTP-Referer": "https://zypercode.com",
          "X-Title": "ZyperCode",
        },
        fetch: async (url, options) => {
          if (options?.body && typeof options.body === "string") {
            try {
              ...
              // Instruct OpenRouter to automatically fall back to alternative healthy providers if one fails
              body.provider = {
              ...
              /* ignore parse errors and proceed with original body */
            }
          }
          return localProxyFetch(url, options);
        },
      })(resolvedModelId);
      break;
    }
...
const DIRECT_CODEGEN_SYSTEM_PROMPT = `You are ZyperCode, an expert coding assistant in a developer IDE.
In this mode, automated tool calls are disabled for this model endpoint.
Rules:
- When asked to create, edit, or write code/files, provide the complete, clean, production-ready code directly in your response.
- Use markdown code blocks with clear language tags and mention the target filepath or filename before or above each code block.
...
function buildStableSystem(
  ...
  const base = disableTools
    ? DIRECT_CODEGEN_SYSTEM_PROMPT
...
export async function runAgentStream(opts: RunAgentOptions) {
  ...
  const isToolIncompatible =
    !opts.disableTools &&
    Boolean(
      (modelId === "openrouter-custom" && opts.openrouterModelId?.includes("glm")) ||
      modelId.toLowerCase().includes("glm") ||
      (opts.openrouterModelId?.includes("r1:free") && !opts.openrouterModelId?.includes("distill")),
    );
  const effectiveDisableTools = opts.disableTools || isToolIncompatible;
  ...
  return streamText({
    ...
    tools: effectiveDisableTools ? undefined : buildTools(opts.toolContext),
