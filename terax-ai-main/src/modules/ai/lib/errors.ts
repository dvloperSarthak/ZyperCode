...
function sanitizeErrorMessage(message: string): string {
  ...
  if (/provider returned error/i.test(redacted)) {
    redacted =
      "The upstream provider returned an error (HTTP 502). The selected model may be experiencing downtime or does not support tool execution. Use the buttons below to switch to a stable model or retry.";
  }
...
function errorPrefix(code: string | null, message: string): string | null {
  ...
  if (/provider returned error|failed after \d+ attempts/i.test(message)) {
    return "AI Provider Error (502)";
  }
