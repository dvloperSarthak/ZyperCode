...
const STORE_PATH = "zypercode-ai-snippets.json";
const LEGACY_STORE_PATH = "terax-ai-snippets.json";
const KEY_LIST = "snippets";

const store = new LazyStore(STORE_PATH, { defaults: {}, autoSave: 200 });

export async function loadSnippets(): Promise<Snippet[]> {
  const current = await store.get<Snippet[]>(KEY_LIST);
  if (current && current.length > 0) return current;
  try {
    const legacy = await new LazyStore(LEGACY_STORE_PATH).get<Snippet[]>(KEY_LIST);
    if (legacy && legacy.length > 0) {
      await store.set(KEY_LIST, legacy);
      await store.save();
      return legacy;
    }
  } catch {}
