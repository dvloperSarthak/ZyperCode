...
async function respond(
  ...
  if (!delivered) {
    console.warn(`[zypercode] control response expired: ${requestId}`);
  }
}
  ...
  useEffect(() => {
    ...
    const handleRequest = async (request: ControlRequest) => {
      try {
        ...
        if (request.method === "open") {
          ...
          if (open.focus) {
            ...
            try {
              ...
              console.warn("[zypercode] could not focus control target:", error);
            }
          }
          ...
          if (tabId === null) {
            throw new RequestError(
              "open_failed",
              "ZyperCode could not create an editor tab",
            );
          }
        ...
        await respond(request.id, { ok: false, error: responseError }).catch(
          (responseError) => {
            console.error("[zypercode] control response failed:", responseError);
          },
        );
      }
    };

    const attach = (evt: string) =>
      listen<ControlRequest>(evt, (event) => {
        void handleRequest(event.payload);
      });

    Promise.all([
      attach("zypercode:control-request").then((stop) => {
        if (disposed) stop();
        else unsubs.push(stop);
      }),
      attach("terax:control-request").then((stop) => {
        if (disposed) stop();
        else unsubs.push(stop);
      }).catch(() => {}),
    ])
      .then(() => {
        if (!disposed) return setFrontendReady(true);
      })
      .catch((error) => {
        console.error("[zypercode] control bridge setup failed:", error);
      });
    ...
    return () => {
      ...
      void setFrontendReady(false).catch((error) => {
        console.error("[zypercode] control bridge cleanup failed:", error);
      });
    };
