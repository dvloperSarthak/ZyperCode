...
export function useEditorFileSync({ tabs, tabsRef, editorRefs }: Params) {
  ...
  useEffect(() => {
    const handleOpenFileWritten = (e: Event) => {
      const customEvent = e as CustomEvent<{ path: string }>;
      if (customEvent.detail?.path) {
        const normalizedPath = customEvent.detail.path.replace(/\\/g, "/");
        // Give time for tab to mount if newly created, then reload
        setTimeout(() => {
    ...
    window.addEventListener("zypercode:open-file", handleOpenFileWritten);
    return () =>
      window.removeEventListener("zypercode:open-file", handleOpenFileWritten);
  }, [tabsRef, editorRefs]);
