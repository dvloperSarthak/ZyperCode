...
export const FileExplorer = memo(
  forwardRef<FileExplorerHandle, Props>(function FileExplorer(
    ...
    useEffect(() => {
      const handleOpen = () => setFolderDialogOpen(true);
      window.addEventListener("terax:open-folder-dialog", handleOpen);
      return () => window.removeEventListener("terax:open-folder-dialog", handleOpen);
    }, []);
      ...
      >
        <div className="flex h-8 shrink-0 items-center gap-1 border-b border-border/60 px-2">
          <button
            ...
            className="flex flex-1 items-center truncate text-xs font-medium text-foreground/80 hover:text-foreground transition-colors group cursor-pointer text-left"
            title={`Click to open another folder\nCurrent: ${rootPath}`}
          >
            <img
            ...
            />
            <span className="truncate group-hover:underline underline-offset-2">{basename(rootPath)}</span>
          </button>
