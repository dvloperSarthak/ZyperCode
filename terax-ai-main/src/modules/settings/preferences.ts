...
const FAST_BG_KIND_KEY = "zypercode-ui-bg-kind-shadow";
const LEGACY_FAST_BG_KIND_KEY = "terax-ui-bg-kind-shadow";
const FAST_BG_IMAGE_ID_KEY = "zypercode-ui-bg-image-shadow";
...
function mirrorBgFastPath(
  ...
  try {
    ...
    window.localStorage.setItem(LEGACY_FAST_BG_KIND_KEY, kind);
    if (imageId) {
      window.localStorage.setItem(FAST_BG_IMAGE_ID_KEY, imageId);
      window.localStorage.setItem(LEGACY_FAST_BG_IMAGE_ID_KEY, imageId);
    } else {
      window.localStorage.removeItem(FAST_BG_IMAGE_ID_KEY);
      window.localStorage.removeItem(LEGACY_FAST_BG_IMAGE_ID_KEY);
    }
  } catch {
    /* ignore */
  }
...
export function readBgFastPath(): {
  ...
  try {
    const kind = window.localStorage.getItem(FAST_BG_KIND_KEY) ?? window.localStorage.getItem(LEGACY_FAST_BG_KIND_KEY);
    const imageId = window.localStorage.getItem(FAST_BG_IMAGE_ID_KEY) ?? window.localStorage.getItem(LEGACY_FAST_BG_IMAGE_ID_KEY);
