...
export const DEFAULT_THEME_ID = "zypercode-default";
...
export type Preferences = {
  ...
  agentAutoApproveFiles: boolean;
  agentAutoApproveCommands: boolean;
...
const STORE_PATH = "zypercode-settings.json";
...
const KEY_AGENT_AUTONOMOUS_MODE = "agentAutonomousMode";
const KEY_AGENT_AUTO_APPROVE_FILES = "agentAutoApproveFiles";
const KEY_AGENT_AUTO_APPROVE_COMMANDS = "agentAutoApproveCommands";
...
export const DEFAULT_PREFERENCES: Preferences = {
  ...
  agentAutonomousMode: false,
  agentAutoApproveFiles: false,
  agentAutoApproveCommands: false,
...
// window can listen.
const PREFS_CHANGED_EVENT = "zypercode://prefs-changed";
const LEGACY_PREFS_CHANGED_EVENT = "terax://prefs-changed";

async function writePref<T>(key: string, value: T): Promise<void> {
  await store.set(key, value);
  await store.save();
  await emit(PREFS_CHANGED_EVENT, { key, value });
  await emit(LEGACY_PREFS_CHANGED_EVENT, { key, value }).catch(() => {});
}
...
export async function loadPreferences(): Promise<Preferences> {
  ...
  if (entries.length === 0) {
    try {
      const legacyStore = new LazyStore(LEGACY_STORE_PATH, { defaults: {}, autoSave: 200 });
      const legacyEntries = await legacyStore.entries();
  ...
  const resolvedThemeId = rawThemeId === "terax-default" ? DEFAULT_THEME_ID : (rawThemeId ?? DEFAULT_PREFERENCES.themeId);
  return {
    ...
    agentAutonomousMode:
      get<boolean>(KEY_AGENT_AUTONOMOUS_MODE) ??
      DEFAULT_PREFERENCES.agentAutonomousMode,
    agentAutoApproveFiles:
      get<boolean>(KEY_AGENT_AUTO_APPROVE_FILES) ??
      DEFAULT_PREFERENCES.agentAutoApproveFiles,
    agentAutoApproveCommands:
      get<boolean>(KEY_AGENT_AUTO_APPROVE_COMMANDS) ??
      DEFAULT_PREFERENCES.agentAutoApproveCommands,
...
export async function setAgentAutonomousMode(value: boolean): Promise<void> {
  await writePref(KEY_AGENT_AUTONOMOUS_MODE, value);
}

export async function setAgentAutoApproveFiles(value: boolean): Promise<void> {
  await writePref(KEY_AGENT_AUTO_APPROVE_FILES, value);
}

export async function setAgentAutoApproveCommands(
  value: boolean,
): Promise<void> {
  await writePref(KEY_AGENT_AUTO_APPROVE_COMMANDS, value);
}
...
export async function onPreferencesChange(
  ...
  const unsubLegacyEvent = await listen<{ key: string; value: unknown }>(
    LEGACY_PREFS_CHANGED_EVENT,
    (e) => {
      const mapped = map[e.payload.key];
      if (mapped) cb(mapped, e.payload.value);
    },
...
const KEYS_CHANGED_EVENT = "zypercode://ai-keys-changed";
const LEGACY_KEYS_CHANGED_EVENT = "terax://ai-keys-changed";

export async function emitKeysChanged(): Promise<void> {
  await emit(KEYS_CHANGED_EVENT);
  await emit(LEGACY_KEYS_CHANGED_EVENT).catch(() => {});
}

export async function onKeysChanged(cb: () => void): Promise<UnlistenFn> {
  const unsub = await listen(KEYS_CHANGED_EVENT, () => cb());
  const unsubLegacy = await listen(LEGACY_KEYS_CHANGED_EVENT, () => cb()).catch(() => () => {});
  return () => {
    unsub();
    unsubLegacy();
  };
