      ...
      <aside className="flex h-full min-w-0 flex-col [contain:layout_style]">
        ...
        {panelState === "no-repo" ? (
          <PanelCenter
            ...
            body="The active workspace folder is not currently a Git repository."
            action={
              <Button
                size="sm"
                onClick={async () => {
                  try {
                    ...
                    const res = await native.runCommand("git init", cwd);
                    if (res.exit_code === 0 || !res.exit_code) {
                      toast.success("Initialized Git repository");
                      await scm.refresh();
                    } else {
                      toast.error(res.stderr || "Failed to initialize Git repository");
                    }
                  } catch (err) {
                    toast.error(`Git init failed: ${String(err)}`);
                  }
                }}
                className="gap-1.5"
              >
