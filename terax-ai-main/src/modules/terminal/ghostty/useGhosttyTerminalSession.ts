...
export async function ghosttyLeafHasForegroundProcess(
  ...
  try {
    ...
    console.error(
      "[zypercode] Ghostty pty_has_foreground_process failed for leaf",
      leafId,
      error,
    );
    return false;
...
function ensureSession(
  ...
  const session: GhosttySession = {
    ...
    writer: new BoundedPtyWriter((error) => {
      console.error("[zypercode] Ghostty PTY write failed:", error);
    }),
    container: null,
    callbacks: {},
    visible: false,
...
function initializeSession(session: GhosttySession): Promise<void> {
  return startSessionInitialization(
    ...
    (error) => {
      ...
      console.error("[zypercode] Ghostty session initialization failed:", error);
...
async function initializeSessionGeneration(
  ...
  const semanticEvents = new GhosttySemanticEventRouter({
    ...
    onOverflow: (dropped) => {
      console.warn(
        `[zypercode] Ghostty semantic event queue dropped ${dropped} event(s)`,
      );
    },
  ...
  if (webGpuPreload.error) {
    console.warn(
      "[zypercode] WebGPU initialization failed; preserving the Ghostty session with WebGL:",
  ...
  if (session.visible && session.container) {
    try {
      ...
      webGpuFallbackStarted = true;
      console.warn(
        "[zypercode] WebGPU surface attachment failed; preserved the Ghostty session with WebGL:",
...
function applyBlockInputMode(
  ...
  const changed =
    input.getAttribute("data-zypercode-block-prompt") !== String(prompt) &&
    input.getAttribute("data-terax-block-prompt") !== String(prompt);
  if (!changed) return;
  input.setAttribute("data-zypercode-block-prompt", String(prompt));
...
async function fallbackWebGpuSurface(
  ...
  console.warn(
    "[zypercode] WebGPU renderer failed; preserved the live Ghostty model, PTY, and scrollback with WebGL:",
    cause.message,
  );
}
...
function logSurfaceError(backend: GhosttyBackend, error: Error): void {
  console.error(
    `[zypercode] ${backend} surface failed:`,
    error.message,
    error.stack ?? error,
  );
}
