...
function connect(): void {
  ...
  if (typeof window !== "undefined" && "__TAURI_INTERNALS__" in window) {
    ...
    const sub = (evtName: string) =>
      win.listen<NativePresentation>(evtName, ({ payload }) => accept(payload));
    void Promise.all([sub("zypercode:window-presentation"), sub("terax:window-presentation")])
      .then(async (stops: (() => void)[]) => {
        if (disposed) {
          stops.forEach((s) => s());
          return;
        }
        unlisten = () => stops.forEach((s) => s());
        accept(await invoke<NativePresentation>("window_presentation_state"));
      })
      .catch((error: unknown) => {
        if (!disposed)
          console.warn(
            "[zypercode] Native window presentation tracking unavailable:",
            error,
          );
      });
  }
