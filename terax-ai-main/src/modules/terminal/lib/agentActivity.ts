...
export function ensureAgentActivityListener(): Promise<void> {
  ...
  const handler = (e: { payload: AgentSignal }) => {
  ...
  listenerReady = Promise.all([
    listen<AgentSignal>("zypercode:agent-signal", handler),
    listen<AgentSignal>("terax:agent-signal", handler).catch(() => () => {}),
  ]).then(
    () => {},
    (error: unknown) => {
      listenerReady = null;
