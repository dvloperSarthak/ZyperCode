...
const FAST_PATH_KEY = "zypercode-ui-theme-shadow";
const LEGACY_FAST_PATH_KEY = "terax-ui-theme-shadow";
const FAST_PATH_THEME_ID = "zypercode-ui-theme-id-shadow";
const LEGACY_FAST_PATH_THEME_ID = "terax-ui-theme-id-shadow";

function readFastMode(fallback: ThemePref): ThemePref {
  if (typeof window === "undefined") return fallback;
  const v = window.localStorage.getItem(FAST_PATH_KEY) ?? window.localStorage.getItem(LEGACY_FAST_PATH_KEY);
...
function writeFastMode(t: ThemePref): void {
  try {
    ...
    window.localStorage.setItem(LEGACY_FAST_PATH_KEY, t);
  } catch { /* ignore */ }
}

function readFastThemeId(): string {
  if (typeof window === "undefined") return DEFAULT_THEME_ID;
  const id = window.localStorage.getItem(FAST_PATH_THEME_ID) ?? window.localStorage.getItem(LEGACY_FAST_PATH_THEME_ID);
  if (!id || id === "terax-default") return DEFAULT_THEME_ID;
  return id;
}
...
function writeFastThemeId(id: string): void {
  try {
    ...
    window.localStorage.setItem(LEGACY_FAST_PATH_THEME_ID, id);
  } catch { /* ignore */ }
}
