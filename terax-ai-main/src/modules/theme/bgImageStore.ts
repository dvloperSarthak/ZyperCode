const DB_NAME = "zypercode-bg-images";
...
function openDb(dbName = DB_NAME): Promise<IDBDatabase> {
  if (dbName === DB_NAME && dbPromise) return dbPromise;
...
export async function getBgImage(id: string): Promise<Blob | null> {
  ...
  try {
    ...
    const legacyRes = await new Promise<Blob | null>((resolve, reject) => {
    ...
    if (legacyRes) {
      void putBgImage(id, legacyRes).catch(() => {});
      return legacyRes;
    }
  } catch {}
  return null;
