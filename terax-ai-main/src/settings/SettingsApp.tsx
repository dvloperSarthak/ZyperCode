...
export function SettingsApp() {
  ...
  useEffect(() => {
    ...
    const unsubs: (() => void)[] = [];
    Promise.all([
      w.listen<string>("zypercode:settings-tab", (e) => apply(e.payload)).then((u) => unsubs.push(u)),
      w.listen<string>("terax:settings-tab", (e) => apply(e.payload)).then((u) => unsubs.push(u)).catch(() => {}),
    ]).catch(() => {});
    return () => {
      unsubs.forEach((u) => u());
    };
  ...
  const handleBackToEditor = useCallback(async () => {
    try {
      ...
      // Ignored if not in Tauri
    }
    // Web / browser dev mode or single-window navigation fallback
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-background text-foreground select-none">
      ...
      >
        <Button
          variant="ghost"
          size="sm"
          onClick={() => void handleBackToEditor()}
          className="h-7 gap-1.5 px-2 text-xs text-muted-foreground hover:text-foreground hover:bg-accent/60 transition-colors shrink-0"
