...
import {
  ...
  setAgentAutonomousMode,
  setAgentAutoApproveFiles,
  setAgentAutoApproveCommands,
...
export function AgentsSection() {
  ...
  const agentAutonomousMode = usePreferencesStore((s) => s.agentAutonomousMode);
  const agentAutoApproveFiles = usePreferencesStore(
    (s) => s.agentAutoApproveFiles,
  );
  const agentAutoApproveCommands = usePreferencesStore(
    (s) => s.agentAutoApproveCommands,
  );
    ...
    <div className="flex flex-col gap-7">
      ...
      <section className="flex flex-col gap-2">
        <div className="flex flex-col">
          <Label>Agent Permissions & Approvals</Label>
          <span className="text-[10.5px] text-muted-foreground">
            Configure how ZyperCode requests your permission before executing modifications or terminal commands.
          </span>
        </div>
        <div className="flex flex-col gap-2">
          <SettingRow
            title="Autonomous Mode"
            description="Allow the agent to operate autonomously without pausing for tool approval confirmations."
          >
            <Switch
              checked={agentAutonomousMode}
              onCheckedChange={(v) => void setAgentAutonomousMode(v)}
            />
          </SettingRow>
          <SettingRow
            title="Auto-Approve File Operations"
            description="Automatically apply file edits, new file creation, and directory modifications without prompting."
          >
            <Switch
              checked={agentAutoApproveFiles || agentAutonomousMode}
              disabled={agentAutonomousMode}
              onCheckedChange={(v) => void setAgentAutoApproveFiles(v)}
            />
          </SettingRow>
          <SettingRow
            title="Auto-Approve Shell Commands"
            description="Automatically run development terminal commands (lint, test, build, scripts) without prompting for approval."
          >
            <Switch
              checked={agentAutoApproveCommands || agentAutonomousMode}
              disabled={agentAutonomousMode}
              onCheckedChange={(v) => void setAgentAutoApproveCommands(v)}
            />
          </SettingRow>
        </div>
      </section>
        ...
        <div className="-mx-2 max-h-[calc(100vh-14rem)] overflow-y-auto px-2 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Textarea
              ...
              placeholder="Persona & rules. Appended to ZyperCode's core system prompt."
