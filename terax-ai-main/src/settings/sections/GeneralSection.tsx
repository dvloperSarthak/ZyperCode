...
export function GeneralSection() {
  ...
  useEffect(() => {
    void invoke<ShellInfo[]>("pty_list_shells")
      .then((res) => setShells(res ?? []))
      .catch(() => setShells([]));
    void invoke<{ name: string }[]>("wsl_list_distros")
      .then((res) => setWslDistros(res ?? []))
      .catch(() => setWslDistros([]));
  }, []);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        {((wslDistros?.length ?? 0) > 0 || defaultWorkspaceEnv !== "local") && (
          <SettingRow
      ...
      <div className="flex flex-col gap-2">
        ...
        <SettingRow
          ...
          description="Alert when a coding agent needs your input or finishes. Native notification when ZyperCode is unfocused, in-app otherwise."
        >
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          <SettingRow
            ...
            description="Open ZyperCode automatically when you sign in."
          >
            <Switch
              checked={autostart}
...
function notificationTestTitle(status: NotificationTestState): string {
  switch (status) {
    ...
    case "failed":
      return "ZyperCode could not request a native notification";
    default:
