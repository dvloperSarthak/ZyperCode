...
export function ModelsSection() {
  ...
  return (
    <div className="flex flex-col gap-7">
      <SectionHeader
        title="Models"
        description="Connect the providers you use. Keys live in your OS keychain and are used only by ZyperCode."
      />
...
const DEFAULT_OPENROUTER_FREE_MODELS: OpenRouterFreeModel[] = [
  { id: "meta-llama/llama-3.3-70b-instruct:free", name: "Meta: Llama 3.3 70B (free - tool use supported)", contextLength: 131072 },
  { id: "google/gemini-2.0-flash-exp:free", name: "Google: Gemini 2.0 Flash (free - tool use supported)", contextLength: 1048576 },
  { id: "qwen/qwen-2.5-coder-32b-instruct:free", name: "Qwen: Qwen 2.5 Coder 32B (free - coding)", contextLength: 32768 },
  { id: "mistralai/mistral-small-24b-instruct-2501:free", name: "Mistral: Mistral Small 24B (free)", contextLength: 32768 },
...
async function fetchOpenRouterFreeModels(): Promise<OpenRouterFreeModel[]> {
  try {
    const res = await fetch("https://openrouter.ai/api/v1/models");
    if (!res.ok) return DEFAULT_OPENROUTER_FREE_MODELS;
    ...
    if (!Array.isArray(json?.data)) return DEFAULT_OPENROUTER_FREE_MODELS;
    ...
    if (free.length === 0) return DEFAULT_OPENROUTER_FREE_MODELS;
    return free.map((m) => ({
      id: m.id,
      name: m.name || m.id,
...
function LocalProviderCard({
  ...
  const [freeModels, setFreeModels] = useState<OpenRouterFreeModel[]>(
  ...
  useEffect(() => {
    ...
    void fetchOpenRouterFreeModels().then((models) => {
      if (!alive) return;
      setFreeModels(models);
      setScrapedCount(models.length);
  ...
  const isCurrentModelFree =
    freeModels.some((m) => m.id === modelDraft) ||
    modelDraft.endsWith(":free") ||
    modelDraft === "openrouter/free";
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border/60 bg-card/60 px-3 py-2.5">
      ...
      <div className="mt-0.5 flex flex-col gap-2.5">
        ...
        {isOpenRouter ? (
          <div className="flex flex-col gap-2 rounded-lg border border-emerald-500/25 bg-emerald-500/5 p-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11.5px] font-medium text-emerald-600 dark:text-emerald-400">
                  Auto-scraped Free Models
                </span>
                {scrapedCount !== null ? (
                  <Badge
                    variant="outline"
                    className="h-4 border-emerald-500/30 bg-emerald-500/10 px-1.5 text-[9.5px] font-normal text-emerald-700 dark:text-emerald-300"
                  >
                    {scrapedCount} free models
                  </Badge>
                ) : null}
              </div>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => void scrapeModels()}
                disabled={fetchingFree}
                className="h-6 gap-1 px-2 text-[10.5px] text-muted-foreground hover:text-foreground"
                title="Auto-scrape latest free models from OpenRouter"
              >
                <HugeiconsIcon
                  icon={ArrowReloadHorizontalIcon}
                  size={11}
                  className={fetchingFree ? "animate-spin" : ""}
                />
                <span>{fetchingFree ? "Scraping…" : "Refresh"}</span>
              </Button>
            </div>

            <Select
              value={
                freeModels.some((m) => m.id === modelDraft) ? modelDraft : ""
              }
              onValueChange={(val) => {
                setModelDraft(val);
                void setModelId(val);
            ...
            >
              <SelectTrigger className="h-8 w-full bg-background/80 font-mono text-[11px]">
                <SelectValue placeholder="Select a free OpenRouter model…" />
              </SelectTrigger>
                  ...
                  >
                    <div className="flex w-full items-center justify-between gap-3">
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        <span className="rounded bg-emerald-500/15 px-1 py-0.5 text-[9px] font-semibold text-emerald-700 dark:text-emerald-400">
                          FREE
                        </span>
                      </div>
                    </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
              ...
              {[
                { id: "meta-llama/llama-3.3-70b-instruct:free", label: "Llama 3.3 70B (Tools)" },
                ...
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setModelDraft(quick.id);
                    void setModelId(quick.id);
                  }}
                  className={cn(
        ...
        <FieldRow label="Model ID">
          <div className="relative flex flex-1 items-center">
            <Input
              ...
              className="h-8 flex-1 font-mono text-[11.5px]"
            />
            {isOpenRouter && isCurrentModelFree && modelDraft ? (
              <span className="pointer-events-none absolute right-2 flex items-center rounded bg-emerald-500/15 px-1.5 py-0.5 text-[9px] font-semibold text-emerald-700 dark:text-emerald-400">
                FREE
              </span>
            ) : null}
          </div>
